...
cv::Mat makeAllBlackPixelsBlue(cv::Mat image) {
    ...
    for (int i = 0; i < image.cols; i ++) {
        for (int i1= 0; i1 < image.rows; i1 ++) {
            cv::Vec3b color = image.at<cv::Vec3b>(i, i1);
            if (blue == 0 && green == 0 && red == 0) {

                image.at<cv::Vec3b>(i, i1) = cv::Vec3b(255, 0, 0);
            }
        }
    }
...
cv::Mat addBackgroundInsteadOfBlackPixelsLargeBackground(cv::Mat object, cv::Mat largeBackground) {
    ...
    cv::Mat doTask4(cv::Mat object, cv::Mat largeBackground, cv::Vec3b color, double dis) {
        ...
        for (int i = 0; i < object.rows; i++) {
            for (int j = 0; j < object.cols; j++) {
                cv::Vec3d original = object.at<cv::Vec3b>(i, j);
                if (pow(original[0] - color[0], 2) + pow(original[1] - color[1], 2) + pow(original[2] - color[2], 2) <=
                    dis * dis)
